...
import { motion } from "framer-motion"; // <-- FIX 3: Correct import
import { Link } from 'react-router-dom'; // <-- FIX 2: Import Link

const MotionLink = motion(Link); // <-- FIX 2: Create an animatable Link
...
const ServiceCard = ({ service, index }) => {
  ...
  return (
    <MotionLink // <-- FIX 2: Use MotionLink instead of motion.div
      to={service.link} // <-- FIX 2: Add the navigation 'to' prop
    ...
    >
      ...
      <div className='relative z-10 flex items-center gap-6 p-8 transition-all duration-300 group-hover:scale-[1.02]'>
        ...
        <div className='relative bg-gradient-to-br from-[#38b6ff] to-[#000433] p-[2px] rounded-full'>
          <div className='bg-white dark:bg-[#000433] rounded-full p-4 w-[72px] h-[72px] flex justify-center items-center'> {/* Added fixed size for alignment */}
            
            {/* FIX 1: Replaced <img> with <span> to render the emoji */}
    ...
    </MotionLink> // <-- FIX 2: Close the MotionLink
  );
};
