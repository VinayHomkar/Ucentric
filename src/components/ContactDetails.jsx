...
// (This component is already responsive, no changes needed)
const InteractiveButton = ({ children, onClick, className }) => {
    ...
    return (
        <button
            ...
            className={`relative rounded-lg p-4 transition-all duration-300 text-left w-full ${className}`} // Added w-full
            style={{
        ...
        >
            <div
                className={`absolute inset-0 rounded-lg transition-opacity duration-300 z-10 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
                style={dynamicGlowStyle}
            ></div>
            <div
                className={`absolute inset-0 rounded-lg transition-opacity duration-300 z-0 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
                style={{
                    boxShadow: 'inset 0 0 12px rgba(56,182,255,0.3)'
                }}
            ></div>
            <div className="relative z-20 w-full h-full text-left">
                {children}
            </div>
        </button>
    );
...
const ContactDetails = () => {
    ...
    const linkedinLink = "https://www.linkedin.com/company/ucentric-official/about/?viewAsMember=true";
    ...
    return (
        ...
        >
            <div className='relative z-10'>
                {/* This h2 is already responsive with text-center lg:text-left */}
                ...
                {/* This flex-col is already mobile-first, which is perfect */}
                ...
                {/* ★★★ FIX 2: Centered icons on mobile, left-aligned on desktop ★★★ */}
                <div className='mt-10 flex gap-4 justify-center sm:justify-start'>
                    <a href={linkedinLink} target="_blank" rel="noopener noreferrer" className='w-10 h-10 flex items-center justify-center rounded-lg border border-[#0077B5] text-[#0077B5]'>
                        <FiLinkedin className='text-xl' />
                    </a>
                    <a href={instagramLink} target="_blank" rel="noopener noreferrer" className='w-10 h-10 flex items-center justify-center rounded-lg border border-[#DD2A7B] text-[#DD2A7B]'>
                        <FiInstagram className='text-xl' />
                    </a>
                    <a href={twitterLink} target="_blank" rel="noopener noreferrer" className='w-10 h-10 flex items-center justify-center rounded-lg border border-[#1DA1F2] text-[#1DA1F2]'>
                        <FiTwitter className='text-xl' />
                    </a>
                    <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className='w-10 h-10 flex items-center justify-center border border-green-400 text-green-400 rounded-lg'>
